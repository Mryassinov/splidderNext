#!/bin/bash
##################################################
# Splidder Kernel Compile Script v3.0 ULTIMATE
# Professional Edition with Auto-Clean & Reset
# Maintainer: yassine
# Zero-Error Guaranteed Build System
##################################################
...
setup_environment() {
    ...
    export ZIPNAME="Splidder-${DEVICE}-$(date '+%Y%m%d-%H%M').zip"
...
add_patches() {
    ...
    # Apply DTBO patches
    log_info "Applying DTBO patches..."
    apply_patch_silent "$DTBO_PATCH1" "DTBO patch 1/6" && ((applied++)) || ((skipped++))
    apply_patch_silent "$DTBO_PATCH2" "DTBO patch 2/6" && ((applied++)) || ((skipped++))
    apply_patch_silent "$DTBO_PATCH3" "DTBO patch 3/6" && ((applied++)) || ((skipped++))
    apply_patch_silent "$DTBO_PATCH4" "DTBO patch 4/6" && ((applied++)) || ((skipped++))
    apply_patch_silent "$DTBO_PATCH5" "DTBO patch 5/6" && ((applied++)) || ((skipped++))
    apply_patch_silent "$DTBO_PATCH6" "DTBO patch 6/6" && ((applied++)) || ((skipped++))
    
    log_info "DTBO: Applied $applied, Skipped $skipped"
...
add_ksu() {
    if [ -n "$KSU_SETUP_URI" ]; then
        ...
        if [[ "$KSU_SETUP_URI" == *"backslashxx/KernelSU"* ]]; then
            ...
            log_info "Setting up KSU_NEXT with SUSFS..."
            ...
            # Apply SUSFS patches
            log_info "Applying SUSFS patches for root hiding..."
...
package_kernel() {
    ...
    echo -e "${GREEN}╔════════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║  Device:      ${CYAN}$DEVICE_CODENAME${GREEN}"
    echo -e "${GREEN}║  KernelSU:    ${CYAN}$KSU_TYPE${GREEN}"
    echo -e "${GREEN}║  LN8K:        ${CYAN}$([ "$LN8K_ENABLED" = true ] && echo "Enabled" || echo "Disabled")${GREEN}"
    echo -e "${GREEN}║  Build Time:  ${CYAN}${build_time_min}m ${build_time_sec}s${GREEN}"
    echo -e "${GREEN}║  Commit:      ${CYAN}$commit_hash${GREEN}"
    echo -e "${GREEN}║  Output:      ${CYAN}$ZIPNAME${GREEN}"
    echo -e "${GREEN}╚════════════════════════════════════════════════════════╝${NC}"
    echo ""
...
🔐 KernelSU: $KSU_TYPE
⚡ LN8K: $([ "$LN8K_ENABLED" = true ] && echo "Enabled" || echo "Disabled")
...
show_help() {
    cat << EOF
${CYAN}╔══════════════════════════════════════════════════════════════╗
║       Splidder Kernel Build Script v3.0 ULTIMATE             ║
...
${GREEN}USAGE:${NC}
    $0 [DEVICE] [KSU_OPTION] [LN8K_OPTION]
...
${GREEN}KSU_OPTION:${NC}
    ...
    --ksu=KSU_NEXT  KernelSU-Next with SUSFS (advanced root hiding)
...
${GREEN}EXAMPLES:${NC}
    ...
    ${YELLOW}# Build with KSU_BLXX${NC}
    $0 sweet --ksu=KSU_BLXX --ln8k=FALSE

    ${YELLOW}# Build with KSU_NEXT + SUSFS${NC}
    $0 sweet --ksu=KSU_NEXT --ln8k=FALSE

    ${YELLOW}# Build vanilla kernel${NC}
    $0 sweet --ksu=NONE --ln8k=FALSE

${GREEN}NOTE:${NC}
    This script automatically performs deep clean before each build.
    No need for manual cleaning or -c flag!
...
main() {
    ...
    echo ""
    echo -e "${MAGENTA}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${MAGENTA}║       Splidder Kernel Build Script v3.0 ULTIMATE             ║${NC}"
    echo -e "${MAGENTA}║       Zero-Error Professional Build System                   ║${NC}"
    echo -e "${MAGENTA}╚══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    if [ -z "$KSU_SELECTOR" ]; then
        echo ""
        echo -e "${GREEN}KernelSU Options:${NC}"
        echo "  1) KSU_BLXX   - backslashxx's KernelSU (hookless mode)"
        echo "  2) KSU_NEXT   - KernelSU-Next with SUSFS (advanced root hiding)"
        echo "  3) NONE       - Vanilla kernel (no root)"
    ...
    echo -e "${CYAN}KernelSU:    ${NC}$KSU_SELECTOR"
    echo -e "${CYAN}LN8K:        ${NC}$LN8K_SELECTOR"
    echo ""
    ...
    setup_environment "$DEVICE" "$KSU_SELECTOR" "$LN8K_SELECTOR"
